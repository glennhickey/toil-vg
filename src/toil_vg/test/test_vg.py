...
class VGCGLTest(TestCase):
    ...
    def test_chr19_sampleNA12877(self):
        subprocess.check_call(['aws', 's3', 'cp', 's3://cgl-pipeline-inputs/vg_cgl/ci/NA12877.lrc_kir.bam.fq', self.workdir])
        subprocess.check_call(['aws', 's3', 'cp', 's3://cgl-pipeline-inputs/vg_cgl/ci/LRC_KIR_chrom_name_chop_100.vg', self.workdir])
        self.sample_reads = os.path.join(self.workdir, 'NA12877.lrc_kir.bam.fq')
